  ...
  describe('Aggregation operators', () => {
    ...
    describe('$replaceRoot', () => {
      ...
      it('accepts an agg expr', () => {
        // TODO: validate that expr resolves to document?
    ...
    describe('$collStats', () => {
      // TODO: validate that its the first? Why is count in the docs?
    ...
    describe('$currentOp', () => {
      ...
      it('accepts all fields', () => {
        accepts('{$currentOp: { allUsers: true, idleConnections: false }}');
      });
      it('rejects without comma', () => {
        rejects(
          '{ $currentOp: {idleConnections: true idleConnections: true} }'
        );
      });
      it('rejects unknown fields', () => {
    ...
    describe('$geoNear', () => {
      ...
      it('rejects the two required if type wrong', () => {
        rejects('{$geoNear: { near: 1, distanceField: 1 } }');
      });
    ...
    describe('$graphLookup', () => {
      ...
      it('accepts query expr for restrictSearchWithMatch', () => {
        accepts(
          '{$graphLookup: { from: "employees", startWith: "$reportsTo",' +
    ...
    describe('$bucket', () => {
      it('accepts min doc', () => {
        accepts(
          '{$bucket: {' + 'groupBy: "$fieldname",' + 'boundaries: [1,2],' + '}}'
        );
      });
      it('accepts an agg expr for groupBy', () => {
        accepts(
          '{$bucket: {' + 'groupBy: {$abs: 1},' + 'boundaries: [1,2],' + '}}'
        );
      });
      it('accepts full doc', () => {
        accepts(
    ...
    describe('$bucketAuto', () => {
      it('accepts min doc', () => {
        accepts(
          '{$bucketAuto: {' + 'groupBy: "$fieldname",' + 'buckets: 10' + '}}'
        );
      });
      it('accepts an agg expr for groupBy', () => {
        accepts(
          '{$bucketAuto: {' + 'groupBy: {$abs: 1},' + 'buckets: 10' + '}}'
        );
      });
      it('accepts full doc', () => {
        accepts(
    ...
    describe('$listLocalSessions', () => {
      ...
      it('accepts one user/db', () => {
        accepts(
          '{$listLocalSessions: {users: [' + '{user: "anna", db:"test"}' + ']}}'
        );
      });
      it('accepts multiple user/db', () => {
        accepts(
          '{$listLocalSessions: {users: [' +
    ...
    describe('$listSessions', () => {
      ...
      it('accepts one user/db', () => {
        accepts(
          '{$listSessions: {users: [' + '{user: "anna", db:"test"}' + ']}}'
        );
      });
      it('accepts multiple user/db', () => {
        accepts(
    ...
    describe('$addFields', () => {
      ...
      it('accepts one field', () => {
        accepts('{$addFields: {' + 'field1: "value1"' + '}}');
      });
      it('rejects empty', () => {
        rejects('{$addFields: {' + '}}');
      });
      ...
      it('accepts agg expr', () => {
        accepts('{$addFields: {' + 'field1: {$abs: 100},' + '}}');
        it('accepts a nested field', () => {
          accepts('{$addFields: {' + '   field1.subfield: {$abs: 100},' + '}}');
        });
      });
      ...
      context('when supplying conversion operators', () => {
        it('accepts $convert', () => {
          accepts(
            '{$addFields: { name: { $convert: { input: true, to: "int"}}}}'
          );
        });
    ...
    describe('$sort', () => {
      ...
      it('accepts meta sort order', () => {
        accepts('{$sort: {' + "field: {$meta: 'textScore'}" + '}}');
      });
      it('accepts one field', () => {
        accepts('{$sort: {' + 'field: 1,' + '}}');
      });
      it('rejects empty', () => {
        rejects('{$sort: {' + '}}');
      });
      ...
      it('rejects sort number not 1/-1 order', () => {
        rejects('{$sort: {' + 'field: 100' + '}}');
      });
      it('rejects $meta without textScore order', () => {
        rejects('{$sort: {' + 'field: {$meta: "notTextScore"' + '}}');
      });
      it('rejects non $meta document', () => {
        rejects('{$sort: {' + 'field: {$notmeta: "TextScore"' + '}}');
      });
    });

    describe('$match', () => {
      it('accepts a simple document', () => {
        accepts('{$match: {' + 'x: 1, y: {q: 1}, z: "testing"' + '}}');
      });
      it('accepts a nested field', () => {
        accepts('{$match: {' + '"x.y.z": 1, y: {q: 1}, z: "testing"' + '}}');
      });
      it('accepts $or', () => {
        accepts('{$match: {' + '$or: [{x:1}, {y: 1}]' + '}}');
      });
      it('accepts $and', () => {
        accepts('{$match: {' + '$and: [{z: 30}, {r: 99}]' + '}}');
      });
      it('accepts an empty document', () => {
        accepts('{$match: {' + '' + '}}');
      });
    ...
    describe('$project', () => {
      it('accepts a included field', () => {
        accepts('{$project: {' + 'testfield: 1,' + 'testfield2: true' + '}}');
      });
      it('accepts a excluded field', () => {
        accepts('{$project: {' + 'testfield: 0,' + 'testfield: false,' + '}}');
      });
      it('accepts a excluded _id field', () => {
        accepts('{$project: {' + '_id: 0,' + '}}');
      });
      it('accepts an agg expr', () => {
        accepts('{$project: {' + '   field: {"$literal": "testing"},' + '}}');
      });
      it('rejects an arbitrary $ field', () => {
        // TODO: limit to defined operators
        rejects('{$project: {' + 'field: {"$testing": "testing"},' + '}}');
      });
      it('accepts nested field', () => {
        accepts('{$project: {' + '"field.nested": "$new.field.name",' + '}}');
      });
      it('accepts nested field without quotes', () => {
        accepts(
          '{$project: {' + 'field.nested: {"$literal": "testing"},' + '}}'
        );
      });
      ...
      it('accepts backslashes', () => {
        accepts('{$project: {' + '"field?": "$new.field.name",' + '}}');
      });
    ...
    describe('$facet', () => {
      ...
      it('rejects a non-array for stage', () => {
        rejects('{$facet: {' + 'output: {test: 1}' + '}}');
      });
      it('rejects an empty pipeline', () => {
        rejects('{$facet: {' + 'output: []' + '}}');
      });
  ...
  describe('Key syntax', () => {
    describe('field', () => {
      it('rejects fields starting with $', () => {
        rejects('{$addFields: {' + '   $field1: "value1",' + '}}');
        rejects('{$addFields: {' + '   \'$field1\': "value1",' + '}}');
        rejects('{$addFields: {' + '   "$field1": "value1",' + '}}');
      });
      it('rejects fields starting with .', () => {
    ...
    describe('query_expr', () => {
      ...
      it('rejects aggregation operators', () => {
        rejects('{' + '$match: {' + '$abs: 100' + '}}');
      });
      it('rejects nested aggregation operators', () => {
        rejects('{' + '$match: {' + '   x: {$abs: 100}' + '}}');
      });
    ...
    describe('agg_expr', () => {
      ...
      it('rejects query operators', () => {
        rejects('{' + '$addFields: {' + '   x: {$regex: 100}' + '}}');
        rejects('{' + '$addFields: {' + '   x: {"$regex": 100}' + '}}');
        rejects('{' + '$addFields: {' + "   x: {'$regex': 100}" + '}}');
      });
      it('accepts accumulators', () => {
        accepts('{' + '$addFields: {' + '   x: {$sum: 100}' + '}}');
        accepts('{' + '$addFields: {' + '   x: {"$sum": 100}' + '}}');
        accepts('{' + '$addFields: {' + "   x: {'$sum': 100}" + '}}');
      });
      it('rejects nested query operators', () => {
        rejects('{' + '$addFields: {' + '   x: { y: {$regex: 100}}' + '}}');
      });
      it('accepts non-operator fields', () => {
        accepts('{$addFields: {x: 100}}');
      });
      it('rejects arbitrary $', () => {
        rejects('{' + '$addFields: {' + '   x: {$otherfield: 100}' + '}}');
        rejects('{' + '$addFields: {' + '   x: {"$otherfield": 100}' + '}}');
        rejects('{' + '$addFields: {' + "   x: {'$otherfield': 100}" + '}}');
      });
    });
